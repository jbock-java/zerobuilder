...
final class VarLife {
  ...
  List<List<TypeVariableName>> methodParams() {
    ...
    builder.getFirst().addAll(varLifes.getFirst());
  ...
  private static List<List<TypeVariableName>> accLife(List<TypeName> steps, List<TypeVariableName> typeParameters) {
    List<List<TypeVariableName>> builder = emptyLists(steps.size());
