...
final class UpdaterContext {
  ...
  UpdaterContext(GoalContext goal) {
    this.goal = goal;
  }

  ClassName typeName() {
    return goal.builderType.nestedClass(UPDATER_IMPL);
  }
  ...
  private ImmutableList<MethodSpec> updaterMethods() {
    ...
    for (ParameterContext parameter : goal.goalParameters) {
      ...
      MethodSpec spec = methodBuilder(name)
  ...
  Optional<TypeSpec> buildUpdaterImpl() {
    ...
    return Optional.of(classBuilder(typeName())
        .addSuperinterface(goal.contractUpdaterName())
        .addFields(fields())
